...
mcp = MCPServer("Fabrient Engineering", instructions="Fabrient MCP: deterministic engineering algorithms + measured-evidence ML + bounded engineering/LLM orchestration. Never invent measurements or confidence. Every tool returns or preserves evidence, status, and provenance where the engine supports it.")
...
# Shared quality contract applied to every one of the 100 tools. This improves
# the surface without changing the underlying engineering calculation.
QUALITY_IMPROVEMENTS = (
    "stable_operation_identity",
    "bounded_timeout",
    "payload_size_guard",
    "strict_base64_validation_for_uploads",
    "normalized_error_reporting",
    "request_timing",
    "request_id_for_traceability",
    "input_fingerprint_for_reproducibility",
    "evidence_boundary_preservation",
    "structured_result_metadata",
)
...
async def _post(path: str, payload: dict[str, Any] | None = None, timeout: float | None = None, operation_name: str | None = None) -> Any:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            data["_fabrient_meta"].update({
                "operation": operation,
                "request_id": request_id,
                "input_fingerprint": input_fingerprint,
                "latency_ms": elapsed_ms,
                "quality_contract": list(QUALITY_IMPROVEMENTS),
                "evidence_policy": "no invented measurements or confidence",
            })
        return data
