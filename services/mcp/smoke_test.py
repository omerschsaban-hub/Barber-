...
# Deterministic contract fixtures. They validate transport, schemas, routing and
# non-empty structured responses; they are never treated as physical evidence.
...
def fixture(name: str) -> dict:
    ...
    if name in {"validate_material"}:
        return {"material": "PETG"}
    if name in {"validate_machine_envelope"}:
    ...
    if name in {"risk_estimate", "risk_estimate", "final_system_identification", "agent_graph", "agent_step", "engineering_agent_run", "run_bounded_engineering_review", "agent_fleet_run", "llm_engineering_critic", "next_experiment", "propose_next_experiment", "calculate_reverification", "reverification_calculate", "uncertainty_calculate", "sim2real_run", "sim2real_compare", "sim2real_calibrate_and_run"}:
        return {"observations": [{"predicted_mm": 10.0 + i, "measured_mm": 10.01 + i} for i in range(10)], "seed": 42}
    # All remaining tools still receive a non-empty structured payload. A safe
    # evidence gate is a valid result; a transport/server failure is not.
