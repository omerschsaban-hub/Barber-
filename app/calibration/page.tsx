...
export default function Calibration() {
  ...
  function set<K extends keyof Profile>(key: K,value: Profile[K]) {
    ...
    try { localStorage.setItem(PROFILE_KEY,JSON.stringify({...profile,[key]:value})); setSaved(true); } catch { /* optional persistence */ }
  }
  ...
  async function run() {
    ...
    try {
      ...
      const r=await fetch(`${ENGINE}/v1/predict`,{
        ...
        body:JSON.stringify({
          nominal_mm:p.nominal_mm,material:p.material,machine:p.machine,
          process_temperature_c:p.process_temperature_c,ambient_temperature_c:p.ambient_temperature_c,
          nominal_shrinkage_pct:p.shrinkage_pct,shrinkage_uncertainty_pct:p.shrinkage_uncertainty_pct,
          tolerance_lower_mm:-p.tolerance_mm/2,tolerance_upper_mm:p.tolerance_mm/2,
  ...
  return <main className="page">
    ...
    <div className="workspace-head">
      <div>
        <h1 className="title">Better accuracy, less typing.</h1>
        <p className="muted">Keep the technical controls when they improve the answer. Fabrient remembers values you actually set, while clearly separating assumptions from real machine measurements.</p>
      </div>
      <span className={`status ${hasRealProfile?'ok':'warn'}`}>{hasRealProfile&&saved?'SAVED PROFILE':'BASELINE ONLY'}</span>
    </div>

    <section className="panel" style={{marginTop:20}}>
      <div className="eyebrow">READY TO RUN</div>
      <h2>{profileSummary}</h2>
      <p className="muted">Run the deterministic baseline now. It is useful for a starting estimate; calibration becomes more accurate when real observations are added.</p>
      ...
      <button className="button" style={{marginLeft:8}} onClick={()=>setAdvanced(v=>!v)}>{advanced?'Hide technical inputs':'Improve accuracy'}</button>
      <Link className="button" href="/records" style={{marginLeft:8}}>Add real measurements</Link>
    </section>

    {advanced&&<section className="panel" style={{marginTop:14}}>
      <div className="eyebrow">TECHNICAL INPUTS / OPTIONAL</div>
      <p className="muted">These stay available because they can materially improve engineering accuracy. You only need to change them when you have better information.</p>
    ...
    <section className="panel" style={{marginTop:14}}>
      {result?<>
        ...
        <p className="annotation">This is a physics baseline, not a measurement of your machine. Real observations remain required before calibration.</p>
      ...
      </>:<><h2>No baseline yet</h2><p className="muted">Run the baseline, or improve accuracy with real measurements and machine data.</p></>}
    </section>
  </main>
}
