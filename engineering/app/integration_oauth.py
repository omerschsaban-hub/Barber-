...
def _metadata(endpoint: str) -> dict:
    ...
    candidates = [f"{base}/.well-known/oauth-authorization-server", f"{base}/.well-known/oauth-protected-resource/mcp"]
    ...
    for url in candidates:
        try:
            ...
            if data.get("authorization_servers"):
                ar = data["authorization_servers"][0]
                rr = httpx.get(f"{ar.rstrip('/')}/.well-known/oauth-authorization-server", timeout=10)
                rr.raise_for_status()
                return rr.json()
        except Exception as exc:
            last = exc
...
def start(provider: str, authorization: str | None) -> dict:
    ...
    with _pool().connection() as db:
        db.execute("INSERT INTO integration_oauth_states(state_hash,user_id,provider,code_verifier_ciphertext,redirect_uri,client_id,expires_at) VALUES(%s,%s,%s,%s,%s,%s,now()+interval '10 minutes')", (hashlib.sha256(state.encode()).digest(), user["user_id"], provider, _seal(verifier), REDIRECT_URI, client_id,))
    if not client_id:
    ...
    scopes = metadata.get("scopes_supported") or ["read"]
    params = {"response_type": "code", "client_id": client_id, "redirect_uri": REDIRECT_URI, "state": state, "code_challenge": challenge, "code_challenge_method": "S256", "scope": " ".join(scopes)}
...
def complete(code: str, state: str) -> dict:
    ...
    with _pool().connection() as db:
        ...
        metadata = _metadata(MCP_PROVIDERS[row["provider"]]["endpoint"])
        payload = {"grant_type": "authorization_code", "code": code, "redirect_uri": row["redirect_uri"], "client_id": row["client_id"], "code_verifier": _open(row["code_verifier_ciphertext"])}
...
def connection_providers(user_id: str) -> list[str]:
    with _pool().connection() as db:
        rows = db.execute("SELECT provider FROM integration_connections WHERE user_id=%s", (user_id,)).fetchall()
    return [r["provider"] for r in rows]


def connection_summary(user_id: str) -> list[dict[str, object]]:
    with _pool().connection() as db:
        rows = db.execute("SELECT provider,scopes,expires_at,updated_at FROM integration_connections WHERE user_id=%s ORDER BY provider", (user_id,)).fetchall()
...
def disconnect_connection(user_id: str, provider: str) -> None:
    with _pool().connection() as db:
        db.execute("DELETE FROM integration_connections WHERE user_id=%s AND provider=%s", (user_id, provider))
