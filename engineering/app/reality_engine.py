...
ENGINE_VERSION="reality-loop-1.1"; MIN_REAL=8; MIN_HELD_OUT=10
@dataclass(frozen=True)
class Observation:
    predicted: float; measured: float; features: tuple[float,...]=(); group: str="default"; experiment_id: str=""
...
def fit_residual_model(observations):
    ...
    held=_mae(y,preds);q95=float(np.quantile(np.abs(y-preds),.95,method="higher"));train_mae=_mae(y,model.predict(X))
    return {"status":"validated" if n>=MIN_HELD_OUT else "limited","n":n,"groups":int(len(unique)),"model":"physics-plus-ridge-residual","version":ENGINE_VERSION,"validation":validation,"held_out_mae":held,"held_out_q95_abs_error":q95,"training_mae":train_mae,"generalization_gap":max(0.0,held-train_mae),"_model":model}
...
def active_experiment(observations,candidates=None):
    ...
    scored=[]
    for c in candidates:
        s=np.asarray(c.get("sensitivity",[]),float);noise=max(float(c.get("noise",1.0)),1e-9);cost=max(float(c.get("cost",1.0)),1e-9);f=float(np.dot(s,s)/noise) if s.size else float(c.get("uncertainty",0.0));scored.append((f/cost,c,f))
    score,candidate,f=max(scored,key=lambda x:x[0]);return {"status":"proposed","experiment":candidate,"expected_information_gain":f,"information_per_cost":score,"selection":"fisher_information_like_score","execution":"awaiting_external_executor"}
def trust_envelope(observations,fitted):
    if not observations:return {"status":"unknown","empirical_fit_score":0.0,"bounds":{}}
    p=np.asarray([o.predicted for o in observations],float);m=np.asarray([o.measured for o in observations],float);r=np.abs(m-p);q95=float(np.quantile(r,.95)) if len(r)>1 else float(r[0]);score=max(0.0,min(.99,1.0-q95/max(float(np.mean(np.abs(m))),1e-9)))
    return {"status":"validated" if fitted.get("status")=="validated" else "limited","empirical_fit_score":score,"bounds":{"prediction_min":float(np.min(p)),"prediction_max":float(np.max(p)),"observed_min":float(np.min(m)),"observed_max":float(np.max(m)),"held_out_abs_error_q95":float(fitted.get("held_out_q95_abs_error",q95))},"warning":"Empirical score is descriptive, not a probability of correctness or a safety guarantee."}
def autonomous_plan(observations,candidates=None,target_mae=None):
    comparison=compare(observations);fitted=fit_residual_model(observations);trust=trust_envelope(observations,fitted);held=fitted.get("held_out_mae");done=bool(fitted.get("status")=="validated" and (target_mae is None or (held is not None and held<=target_mae)));next_step={"action":"validate_holdout_and_freeze","reason":"Target held-out error reached with independent validation."} if done else active_experiment(observations,candidates);fingerprint=hashlib.sha256(repr([(o.predicted,o.measured,o.features,o.group) for o in observations]).encode()).hexdigest()[:16]
