...
def _insert_observation(
    ...
    if len(encoded.encode("utf-8")) > MAX_OBSERVATION_BYTES:
        raise HTTPException(413, "Observation payload is too large")
    ...
    with transaction() as conn:
        inserted = conn.execute(
            ...
            values(%s,%s,%s,%s,%s,%s,%s::jsonb,%s::jsonb,%s::jsonb,%s,'pending',NULL,%s)
            on conflict(source_key,content_hash) do nothing returning id""",
            (project_id, source_key, observed, entity_type, entity_id, event_type, encoded, encoded, json.dumps(provenance or {}), consent_state, content_hash),
        ).fetchone()
        if inserted:
            return str(inserted["id"]), False
        existing = conn.execute(
            "select id from data_observations where source_key=%s and content_hash=%s",
...
def post(source_key: str, observation: dict[str, Any]) -> dict[str, Any]:
    ...
    observation_id, deduplicated = _insert_observation(
        ...
        consent_state=str(observation.get("consent_state", "not_applicable")),
    )
...
def catalog():
    ...
    return {"count": len(keys), "sources": keys, "configured": bool(keys), "expected_count": len(SOURCES)}
...
def ingest(o: Observation, x_fabrient_ingest_secret: str | None = Header(default=None)):
    ...
    observation_id, deduplicated = _insert_observation(
        o.source_key, o.event_type, payload, project_id=o.project_id,
        entity_type=o.entity_type or "product_request", entity_id=o.entity_id,
        provenance=o.provenance, consent_state=o.consent_state, observed_at=o.observed_at,
    )
...
def seed_catalog(x_fabrient_ingest_secret: str | None = Header(default=None)):
    _auth(x_fabrient_ingest_secret)
    return {"count": _seed(), "expected_count": len(SOURCES)}
