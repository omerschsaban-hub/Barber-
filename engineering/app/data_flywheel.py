...
def _auth(secret: str | None) -> None:
    if INGEST_SECRET and (not secret or not __import__("hmac").compare_digest(secret, INGEST_SECRET)):
...
def ingest(o: Observation, x_fabrient_ingest_secret: str | None = Header(default=None)):
    ...
    content_hash = hashlib.sha256(json.dumps({"s": o.source_key, "e": o.event_type, "p": payload}, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    row = fetch_all("""insert into data_observations(project_id,source_key,observed_at,entity_type,entity_id,event_type,raw_payload,normalized_payload,provenance,consent_state,validation_state,quality_score,content_hash)
                      values(%s,%s,%s,%s,%s,%s,%s::jsonb,%s::jsonb,%s::jsonb,%s,'validated',1.0,%s)
                      on conflict(source_key,event_type,content_hash) do update set observed_at=excluded.observed_at
                      returning id""", (o.project_id, o.source_key, o.observed_at or datetime.now(timezone.utc), o.entity_type, o.entity_id, o.event_type, json.dumps(o.raw_payload), json.dumps(o.normalized_payload), json.dumps(o.provenance), o.consent_state, content_hash))
    return {"accepted": True, "observation_id": str(row[0]["id"]), "source_key": o.source_key, "deduplicated": False}
