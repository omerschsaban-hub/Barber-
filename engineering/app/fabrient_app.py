...
app=FastAPI(title="Fabrient Engineering API",version="1.4.0")
origin=os.getenv("FABRIIENT_WEB_ORIGIN","http://localhost:3000").strip()
app.add_middleware(CORSMiddleware,allow_origins=[origin] if origin else [],allow_methods=["GET","POST"],allow_headers=["content-type","authorization"],allow_credentials=True)
app.include_router(cad_router)
for route in legacy_app.routes: app.router.routes.append(route)
