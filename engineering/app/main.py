...
def physics(x: EngineeringInput) -> tuple[float, float]:
    ...
    # First-order dimensional model. Temperature is retained as context, not silently turned into an invented coefficient.
...
def calibrate(x: CalibrationRequest):
    ...
    # Leave-one-out validation prevents reporting training error as validation performance.
    X = pred.reshape(-1, 1)
...
def uncertainty(physics_sigma_mm: float = Field(default=0.0, ge=0), measurement_sigma_mm: float = Field(default=0.0, ge=0), model_sigma_mm: float = Field(default=0.0, ge=0), n_observations: int = 0):
    total = math.sqrt(physics_sigma_mm**2 + measurement_sigma_mm**2 + model_sigma_mm**2)
    return {"sigma_mm": total, "interval_multiplier": 1.96, "interval_95_mm": [-1.96*total, 1.96*total], "state": "not_calibrated" if n_observations < 3 else "limited", "components": {"physics": physics_sigma_mm, "measurement": measurement_sigma_mm, "model": model_sigma_mm}}
...
def reverification(x: ReverificationInput):
    # Conservative hazard-style heuristic; it is explicitly an engineering decision aid, not a standard or certification.
...
async def step_geometry(file: UploadFile = File(...)):
    ...
    # Functional STEP geometry extraction: parse CARTESIAN_POINT coordinates for a robust model bounding box.
    pts=[]
