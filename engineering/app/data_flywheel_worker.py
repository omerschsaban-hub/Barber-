...
def _audit(agent: str, status: str, inp: dict[str, Any], out: dict[str, Any], started: datetime, run_id: str) -> None:
    execute("""insert into agent_runs(project_id,run_id,agent_type,status,input,output,model,created_at,completed_at)
               values(%s,%s,%s,%s,%s::jsonb,%s::jsonb,'deterministic-worker',%s,%s)""", (SYSTEM_PROJECT_ID,run_id,agent,status,__import__("json").dumps(inp),__import__("json").dumps(out),started,datetime.now(timezone.utc)))


def run_once() -> dict[str, Any]:
    run_id = __import__("secrets").token_hex(16)
    ...
    for agent, timeout_s, retries in AGENTS[1:]:
        ...
        for obs in observations:
            ...
            execute("insert into data_quality_checks(observation_id,check_name,passed,score,details) values(%s,%s,%s,%s,%s::jsonb)",(obs["id"],agent,ok,1.0 if ok else 0.0,__import__("json").dumps(details)))
    ...
    for obs in [o for o in observations if o.get("source_key") in {"prediction_reality","false_positives","false_negatives","engineer_corrections"}][:25]:
        ...
        execute("""insert into improvement_candidates(project_id,source_observation_id,title,hypothesis,evidence,target_component,risk_score,status)
                   values(%s,%s,%s,%s,%s::jsonb,'validation/calibration',1.0,'proposed')""",(SYSTEM_PROJECT_ID,obs["id"],title,"Observed production evidence indicates a recurring prediction/reality or engineering-correction discrepancy.",__import__("json").dumps({"observation_id":str(obs["id"]),"source_key":obs.get("source_key")})))
        result["improvement_candidates"]+=1
    execute("insert into flywheel_checkpoints(baseline_metrics,experiment_metrics,regression_metrics,decision) values(%s::jsonb,%s::jsonb,%s::jsonb,'hold_for_validation')",(__import__("json").dumps({"sources":len(sources),"observations":len(observations)}),__import__("json").dumps(result["agents"]),__import__("json").dumps({"idempotent":True})))
    return result
...
def manual_run(x_fabrient_run_token: str|None=Header(default=None)):
    ...
    if not expected or not x_fabrient_run_token or not __import__("hmac").compare_digest(x_fabrient_run_token,expected): raise HTTPException(401,"Unauthorized")
    return run_once()
