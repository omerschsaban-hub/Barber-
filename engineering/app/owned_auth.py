...
async def _gmail_access_token()->str:
 ...
 async with httpx.AsyncClient(timeout=10) as client: response=await client.post('https://oauth2.googleapis.com/token',data={'client_id':client_id,'client_secret':client_secret,'refresh_token':refresh,'grant_type':'refresh_token'})
 if response.status_code!=200: raise RuntimeError('Gmail OAuth token refresh failed')
...
async def _send_otp_email(email:str,code:str)->None:
 ...
 async with httpx.AsyncClient(timeout=10) as client: response=await client.post('https://gmail.googleapis.com/gmail/v1/users/me/messages/send',headers={'Authorization':f'Bearer {token}','Content-Type':'application/json'},json={'raw':raw})
 if response.status_code>=300: raise RuntimeError('Gmail API rejected the OTP message')
...
def _normalize_email(email:str)->str:
 ...
 if len(normalized)>254 or '@' not in normalized or not normalized.endswith('@gmail.com'): raise HTTPException(400,'Use a valid Gmail address.')
 return normalized
def _bearer(request:Request,authorization:str|None)->str|None:
 header=authorization or request.headers.get('authorization')
 if header and header.lower().startswith('bearer '): return header[7:].strip()
 return request.cookies.get(COOKIE_NAME)
def user_from_token(token:str|None)->dict[str,Any]|None:
 if not token:return None
 return fetch_all("select u.id::text as id,u.email,u.display_name,u.email_verified_at,u.role from sessions s join users u on u.id=s.user_id where s.token_hash=%s and s.revoked_at is null and s.expires_at>now()",(_hash(token),))[0] if fetch_all("select u.id::text as id,u.email,u.display_name,u.email_verified_at,u.role from sessions s join users u on u.id=s.user_id where s.token_hash=%s and s.revoked_at is null and s.expires_at>now()",(_hash(token),)) else None
def _session(token:str|None)->dict[str,Any]|None:
 user=user_from_token(token); return {'user_id':user['id'],'email':user['email'],'display_name':user['display_name'],'role':user['role']} if user else None
...
def verify_otp(body:VerifyRequest,request:Request):
 ...
 with transaction() as conn:
  challenge=conn.execute("select id,code_hash,attempts,expires_at from otp_challenges where lower(email)=lower(%s) and consumed_at is null order by created_at desc limit 1 for update",(email,)).fetchone()
  if not challenge or challenge['expires_at'].timestamp()<=time.time() or int(challenge['attempts'])>=MAX_OTP_ATTEMPTS:
   if challenge: conn.execute('update otp_challenges set consumed_at=now() where id=%s',(challenge['id'],))
   raise HTTPException(400,'That code is invalid or expired.')
  if not hmac.compare_digest(bytes(challenge['code_hash']),_hash(body.code)): conn.execute('update otp_challenges set attempts=attempts+1 where id=%s',(challenge['id'],)); raise HTTPException(400,'That code is invalid or expired.')
  conn.execute('update otp_challenges set consumed_at=now() where id=%s',(challenge['id'],)); user=conn.execute("insert into users(email,email_verified_at) values(%s,now()) on conflict((lower(email))) do update set email_verified_at=coalesce(users.email_verified_at,now()),updated_at=now() returning id::text as id,email,display_name,email_verified_at,role",(email,)).fetchone(); token=secrets.token_urlsafe(48); conn.execute("insert into sessions(user_id,token_hash,expires_at) values(%s,%s,now()+(%s*interval '1 second'))",(user['id'],_hash(token),SESSION_TTL_SECONDS)); conn.execute("insert into audit_logs(user_id,action,resource_type,resource_id,metadata) values(%s,'auth.login','session',%s,%s::jsonb)",(user['id'],user['id'],'{"method":"gmail_otp"}'))
