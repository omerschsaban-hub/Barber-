...
for router in (advanced_router, real_cv_sim2real_router, sim2real_loop_router, cv_json_router, risk_map_router, validate_dimension_router, mcp_compat_fixes_router, manufacturing_router, cad_router, final_router, quality_router, compat_router, data_flywheel_router, data_flywheel_worker_router, moat_intelligence_router, integration_gateway_router, competitive_product_loop_router, customer_obsession_router, owned_auth_router, billing_router):
    app.include_router(router)
install_product_intelligence(app)
install_universal_quality(app)

# The MCP registry is the single source of truth for the 100-tool compatibility
# surface. Parse only its literal registry so the engineering runtime does not
