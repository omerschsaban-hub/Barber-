...
# Engineering honesty: this module never manufactures calibration observations or literature coefficients.
...
def _risk(nominal: float, predicted: float, sigma: float, lo: float, hi: float):
    ...
    return {"risk_level": level, "interval_95_mm": list(interval), "tolerance_consumed_fraction": consumed,
            "supported": level != "refuse", "reason": "Prediction interval intersects or exceeds acceptance bounds." if outside else "Prediction interval remains inside acceptance bounds."}
...
def computed_risk(x: RiskRequest):
    return {"status": "ok", "result": _risk(x.nominal_mm, x.predicted_mm, x.uncertainty_mm, x.lower_tol_mm, x.upper_tol_mm),
            "provenance": {"source": "deterministic_engineering", "uncertainty": "validated input; no fabricated confidence"}}
...
def system_identification(x: SystemIDRequest):
    ...
    if len(machines) != 1:
        return {"status": "refused", "reason": "System identification requires one machine at a time to avoid conflating machine effects.", "machines": machines}
    ...
    resid = y-cv
    sigma = float(max(np.std(resid, ddof=1), 1e-9))
    return {"status": "validated" if mae < max(float(np.std(y, ddof=1))*0.9, 1e-6) else "limited", "machine_id": machines[0],
            "n": len(y), "coefficients": dict(zip(["temperature_c","humidity_pct","layer_height_mm","speed_mm_s"], model.coef_.tolist())),
...
def confirm_import(x: ImportConfirm):
    ...
    if x.unit.lower() not in {"mm", "millimeter", "millimeters"}:
        raise HTTPException(422, "Only explicit millimetre ingestion is enabled in v1; convert and confirm before import.")
    ...
    return {"status":"confirmed", "row_count":len(x.rows), "content_sha256":x.content_sha256, "confirmation_hash":digest,
            "mapping":x.mapping, "synthetic":False, "next":"persist rows transactionally in Supabase using the authenticated project owner."}
...
def inspection_pdf(record: dict[str, Any]):
    ...
    story=[Paragraph("Fabrient Inspection Record", style=None), Spacer(1,10)]
    meta=[[k.replace('_',' ').title(), str(record[k])] for k in required if k != "measurements"]
    story.append(Table(meta))
    ...
    t=Table(rows,repeatRows=1); t.setStyle(TableStyle([("GRID",(0,0),(-1,-1),0.5,colors.black),("BACKGROUND",(0,0),(-1,0),colors.lightgrey)])); story.append(t)
    story.append(Spacer(1,12)); story.append(Paragraph("This record reports measured evidence. It is not a certification unless the user's quality system separately establishes that authority.", style=None))
...
def bounded_agent_step(x: AgentStep):
    ...
    # The agent is deliberately a bounded planner, not an unrestricted executor.
    actions=["observe_real_measurements","validate_provenance","estimate_uncertainty","propose_next_experiment"]
    return {"status":"planned","iterations":min(x.max_iterations,4),"budget":x.budget,"allowed_actions":actions,
