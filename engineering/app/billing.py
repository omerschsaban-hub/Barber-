...
from .postgres import execute, fetch_one
...
async def revenuecat_webhook(request: Request, authorization: str | None = Header(default=None), x_revenuecat_webhook_signature: str | None = Header(default=None)):
    ...
    if user:
        ...
        expiry = datetime.fromtimestamp(float(expires_at) / 1000, tz=datetime.now().astimezone().tzinfo) if expires_at else None
        for entitlement in entitlements:
            execute("""insert into billing_entitlements(user_id,entitlement_id,active,product_id,expires_at,source)
                       ...
                       on conflict(user_id,entitlement_id) do update set active=excluded.active,product_id=excluded.product_id,expires_at=excluded.expires_at,updated_at=now()""", (user["id"], str(entitlement), active, event.get("product_id"), expiry))
